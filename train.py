...
def train_with_Conv(BATCH_SIZE):
    ...
    if torch.cuda.is_available():
        ...
        for iteration in range(0,epoch):
                ...
                '''
                we set the learning rates after playing with different learning rates and number of total epochs(the hyperparameters)
                '''
                if iteration<=50:
                    optimizer = torch.optim.Adam(net.parameters(),weight_decay=1e-4) #default learning rate is .0001
                else:
                    if iteration<=60:
                        optimizer = torch.optim.Adam(net.parameters(),lr=.00001,weight_decay=1e-4)
                    else:
                        optimizer = torch.optim.Adam(net.parameters(),lr=.000001,weight_decay=1e-4)
...
def train_with_MNN(BATCH_SIZE):
    epoch=50 #we set the learning rates after playing with different learning rates and number of total epochs(the hyperparameters)
    ...
    if torch.cuda.is_available():
        ...
        for iteration in range(0,epoch):
                '''
                if(iteration==0):
                    if os.path.exists(PATHMNN):
                        net.load_state_dict(torch.load(PATHMNN)['model_state_dict']) 
                '''
                '''
                we set the learning rates after playing with different learning rates and number of total epochs(the hyperparameters)
                '''
                ...
                if(iteration>=10):
                    optimizer = torch.optim.Adam(net.parameters(),lr=.00001,weight_decay=1e-4)
                if(iteration>=20):
                    optimizer = torch.optim.Adam(net.parameters(),lr=.0000001,weight_decay=1e-4)
